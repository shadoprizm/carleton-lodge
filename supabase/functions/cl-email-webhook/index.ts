...
const verifySvixWebhook = async (
  ...
  const signed = new TextEncoder().encode(`${messageId}.${timestamp}.${rawBody}`);
...
Deno.serve(async (req: Request) => {
  ...
  if (!webhookSecret || !apiKey || !supabaseUrl || !serviceRoleKey) {
    return jsonResponse({ error: "Email webhook environment is incomplete" }, 503);
  }
  if (!["resend", "agentmail"].includes(provider)) {
    return jsonResponse({ error: `Unsupported EMAIL_PROVIDER: ${provider}` }, 503);
  }
  ...
  if (provider === "resend") {
    const response = await fetch(
      `https://api.resend.com/emails/receiving/${encodeURIComponent(providerMessageId)}`,
      {
    ...
    if (!response.ok) {
      return jsonResponse({ error: `Could not retrieve received email (${response.status})` }, 502);
    }
  ...
  const { error } = await supabase
    ...
    .upsert({
      ...
      attachments: Array.isArray(message.attachments)
        ...
        : Array.isArray(webhookData.attachments) ? webhookData.attachments.slice(0, 100) : [],
      raw_payload: event,
      received_at: String(message.created_at ?? webhookData.created_at ?? event.created_at ?? new Date().toISOString()),
    }, {
