...
Deno.test("MXroute provider preserves an existing mailbox during create", async () => {
  ...
  const provider = createMxrouteProvider({
    ...
    fetch: (input, init) => {
      ...
      requests.push({ url: String(input), method: requestInit?.method ?? "GET" });
...
Deno.test("MXroute provider creates a missing mailbox and verifies it", async () => {
  ...
  const provider = createMxrouteProvider({
    ...
    fetch: (_input, init) => {
      ...
      requests.push({ method, body: typeof requestInit?.body === "string" ? requestInit.body : "" });
      ...
      return Promise.resolve(new Response(JSON.stringify({
        data: { email: "secretary@carpmasons.ca", quota: 500, usage: 0, limit: 200, sent: 0, suspended: false },
      }), { status: 200, headers: { "Content-Type": "application/json" } }));
    },
  });
  ...
  assertEquals(requests.map(request => request.method), ["GET", "POST", "GET"]);
  ...
  const provider = createMxrouteProvider({
    ...
    fetch: (_input, init) => {
      ...
      requests.push({
        ...
        body: typeof requestInit?.body === "string" ? JSON.parse(requestInit.body) : {},
      });
...
Deno.test("MXroute provider surfaces provider failure instead of reporting success", async () => {
  ...
  let message = "";
  try {
    await provider.updateMailbox("webmaster@carpmasons.ca", { password: "MemberChosen123" });
  } catch (error) {
