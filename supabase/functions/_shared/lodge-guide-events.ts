...
const EVENT_QUESTION_PATTERN =
  /\b(event|events|meeting|meetings|calendar|bbq|barbecue|gathering|gatherings|installation|installations)\b/i;

export const lodgeGuideQuestionNeedsEvents = (question: string) =>
  EVENT_QUESTION_PATTERN.test(question);

export const lodgeGuideTorontoDateTime = (
  now = new Date(),
): LodgeGuideLocalDateTime => {
...
export const lodgeGuideEventIsCurrentOrFuture = (
  ...
  const lastRelevantTime = event.event_end_time ?? event.event_time ??
    "23:59:59";
