...
const eventProperties = {
  ...
  district_name: { type: ["string", "null"], enum: ["Ottawa District 1", "Ottawa District 2", null] },
...
export const mailroomExtractionSchema = {
  ...
  properties: {
    publication_target: { type: "string", enum: ["carleton", "district", "mixed", "hold"] },
    classification_tags: {
      type: "array",
      uniqueItems: true,
    ...
    source_scope: {
      type: "string",
      enum: ["carleton", "district_1", "district_2", "outside_scope", "unknown"],
    },
    source_issuer: { type: "string" },
    ...
    summons: {
      anyOf: [{
        ...
        required: ["destination", "district_lodge_id", "title", "month", "issue_date", "content", "notify_members", "include_in_lodge_guide"],
      }, { type: "null" }],
    },
    district_lodge: {
      anyOf: [{
        ...
        properties: {
          id: { type: "string" },
          district_name: { type: "string", enum: ["Ottawa District 1", "Ottawa District 2"] },
          name: { type: "string" },
        ...
        required: ["id", "district_name", "name", "lodge_number", "location", "website_url", "worshipful_master_name", "secretary_name", "contact_email", "contact_phone", "details_as_of"],
      }, { type: "null" }],
    },
    events: {
      type: "array",
    ...
    announcements: {
      ...
      items: {
        ...
        required: ["title", "body", "notice_type", "priority", "visibility", "expires_at", "notify_members", "include_in_lodge_guide", "source_issuer"],
      },
    },
    library_items: {
      type: "array",
      ...
      items: {
        ...
        required: ["title", "summary", "source", "source_url", "source_file_name", "tags", "rights_reviewed", "include_in_lodge_guide"],
      },
    },
    warnings: { type: "array", maxItems: 30, items: { type: "string" } },
  },
  required: ["publication_target", "classification_tags", "source_scope", "source_issuer", "sensitivity", "needs_attachment_content", "confidence", "summary", "summons", "district_lodge", "events", "announcements", "library_items", "warnings"],
};

export const buildMailroomInstructions = (directory: DirectoryLodge[]) => `You prepare proposed website actions for Carleton Lodge No. 465 in Carp, Ontario.
...
6. A death or memorial notice becomes a members-only memorial announcement. notify_members=false, include_in_lodge_guide=false, and include an expiry when reasonable. Create a service event only when the date, time, and place are explicit; keep it members-only, unnotified, and excluded from Lodge Guide.
...
export const normalizeMailroomProposal = (
  ...
  const tags = Array.isArray(proposal.classification_tags)
    ? proposal.classification_tags.filter((item): item is string => typeof item === "string" && validTags.has(item))
    : ["no_action"];
  ...
  const normalizeLodgeId = (raw: unknown) => {
    ...
    warnings.push("A proposed visiting lodge did not match the approved Ottawa District 1 or 2 directory and was removed.");
    return null;
  };

  const summons = proposal.summons == null ? null : { ...asObject(proposal.summons) };
  if (summons?.destination === "district") {
    ...
    if (!summons.district_lodge_id) {
      warnings.push("The visiting-lodge summons is held because an approved directory match is required.");
    }
  }

  const events = (Array.isArray(proposal.events) ? proposal.events : []).map((item) => {
    const event = { ...asObject(item) };
    if (event.destination === "district") event.district_lodge_id = normalizeLodgeId(event.district_lodge_id);
    if (event.destination === "carleton") {
      event.district_lodge_id = null;
      event.district_name = null;
    }
    if (event.source_issuer == null) event.source_issuer = String(proposal.source_issuer ?? "");
    return event;
  });
  ...
  const districtLodge = directoryLodge
    ? { ...districtLodgeValue, id: directoryLodge.id, name: directoryLodge.name, district_name: directoryLodge.district_name, lodge_number: directoryLodge.lodge_number ?? "", location: directoryLodge.location ?? String(districtLodgeValue.location ?? "") }
    : null;

  const bodySource = sourceFiles.find((file) => file.kind === "email_body") ?? sourceFiles[0] ?? null;
  const libraryItems = (Array.isArray(proposal.library_items) ? proposal.library_items : []).map((item) => {
    const library = { ...asObject(item) };
    const requested = String(library.source_file_name ?? "").toLowerCase();
    const source = sourceFiles.find((file) => String(file.file_name ?? "").toLowerCase() === requested) ?? bodySource;
    return {
      ...library,
  ...
  const sourceScope = ["carleton", "district_1", "district_2", "outside_scope", "unknown"].includes(String(proposal.source_scope))
    ? String(proposal.source_scope)
    : "unknown";
  if (sourceScope === "outside_scope") warnings.push("This material is outside the approved Ottawa District 1 and 2 publication scope.");

  return {
    ...proposal,
    publication_target: sourceScope === "outside_scope" ? "hold" : proposal.publication_target,
    ...
    events,
    announcements: Array.isArray(proposal.announcements) ? proposal.announcements : [],
    library_items: libraryItems,
    source_files: sourceFiles,
    source_file: sourceFiles.find((file) => file.kind === "attachment") ?? bodySource,
