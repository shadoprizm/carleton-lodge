...
export const readHeader = (headers: unknown, name: string) => {
  ...
  if (Array.isArray(headers)) {
    return headers.map(asObject)
      .filter((header) => String(header.name ?? header.key ?? "").toLowerCase() === expected)
...
export const canonicalMessageMaterial = (message: JsonObject) => JSON.stringify({
  from: extractEmailAddress(message.from ?? message.from_address),
  to: asStringArray(message.to ?? message.to_addresses).map(extractEmailAddress).sort(),
  cc: asStringArray(message.cc ?? message.cc_addresses).map(extractEmailAddress).sort(),
  subject: String(message.subject ?? "").trim(),
  text: String(message.text ?? message.text_body ?? "").replace(/\r\n/g, "\n").trim(),
  html: String(message.html ?? message.html_body ?? "").replace(/\s+/g, " ").trim(),
  attachments: (Array.isArray(message.attachments) ? message.attachments : [])
    .map(asObject)
    .map((attachment) => ({
      name: String(attachment.filename ?? attachment.file_name ?? attachment.name ?? ""),
      type: String(attachment.content_type ?? attachment.contentType ?? attachment.mime_type ?? ""),
