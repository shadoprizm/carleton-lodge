export { LODGE_EMAIL_DOMAIN, LODGE_EMAIL_SETUP } from "./lodge-email-settings.ts";
...
function toProviderStatus(address: string, account: MxrouteApiAccount): ProviderMailboxStatus {
  return {
    ...
    suspended: typeof account.suspended === "boolean" ? account.suspended : null,
  };
}
...
export function createMxrouteProvider(options: MxrouteProviderOptions = {}): LodgeEmailProvider {
  const request = async (path: string, init: RequestInit = {}) => {
    const { server, username, apiKey } = options.configuration ?? mxrouteConfiguration();
  ...
  return {
    ...
    async getMailbox(address) {
      ...
      const response = await request(accountPath(normalized), { method: "GET" });
      ...
      if (!body?.data) throw new Error("MXroute returned an invalid mailbox response");
    ...
    async createMailbox(input) {
      ...
      if (!created) throw new Error("MXroute did not return the newly created mailbox");
      return created;
    },
    ...
    async deleteMailbox(address) {
      const response = await request(accountPath(address), { method: "DELETE" });
