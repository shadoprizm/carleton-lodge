...
const isPdfAttachment = (attachment: JsonObject) => {
  const filename = attachmentValue(attachment, "filename", "file_name", "name").toLowerCase();
  const contentType = attachmentValue(attachment, "content_type", "contentType", "mime_type").toLowerCase();
...
async function downloadAttachment(
  ...
  const attachmentId = attachmentValue(attachment, "id", "attachment_id", "attachmentId");
  if (!attachmentId) throw new Error("The PDF attachment has no provider identifier");

  let metadataUrl: string;
  if (provider === "resend") {
    metadataUrl = `https://api.resend.com/emails/receiving/${encodeURIComponent(providerMessageId)}/attachments/${encodeURIComponent(attachmentId)}`;
    ...
    metadataUrl = `https://api.agentmail.to/v0/inboxes/${encodeURIComponent(inboxId)}/messages/${encodeURIComponent(providerMessageId)}/attachments/${encodeURIComponent(attachmentId)}`;
  } else {
  ...
  if (!metadataResponse.ok) {
    throw new Error(`The email provider could not retrieve the PDF (${metadataResponse.status})`);
  }
  const metadata = asObject(await metadataResponse.json());
  const nested = asObject(metadata.data);
  const downloadUrl = String(
    metadata.download_url ?? metadata.downloadUrl ?? nested.download_url ?? nested.downloadUrl ?? "",
  );
  if (!downloadUrl.startsWith("https://")) {
    throw new Error("The email provider did not return a secure attachment URL");
  }

  const fileResponse = await fetch(downloadUrl, { signal: AbortSignal.timeout(20_000) });
  if (!fileResponse.ok) throw new Error(`The PDF download failed (${fileResponse.status})`);
  const contentLength = Number(fileResponse.headers.get("content-length") ?? "0");
  if (contentLength > MAXIMUM_PDF_BYTES) throw new Error("The PDF is larger than 10 MB");
...
const extractionSchema = {
  ...
  properties: {
    ...
    district_lodge: {
      anyOf: [{
        ...
        required: [
          "name", "lodge_number", "location", "website_url",
          "worshipful_master_name", "secretary_name", "contact_email",
          "contact_phone", "details_as_of",
        ],
      }, { type: "null" }],
    },
    events: {
      ...
      items: {
        ...
        properties: {
          ...
          event_kind: {
            type: "string",
            enum: ["meeting", "emergent", "installation", "social", "official_visit", "other"],
          },
          degree: {
            type: "string",
            enum: ["unspecified", "none", "first", "second", "third", "installation", "other"],
          },
          visibility: { type: "string", enum: ["public", "members", "admin"] },
        },
        required: [
          "title", "description", "event_date", "event_time", "event_end_time",
          "location", "location_address", "poc_name", "poc_contact",
          "event_kind", "degree", "visibility",
        ],
      },
    },
    announcements: {
  ...
  required: [
    "publication_target", "classification", "confidence", "summary",
    "summons", "district_lodge", "events", "announcements", "warnings",
  ],
};
...
Deno.serve(async (req: Request) => {
  ...
  if (!supabaseUrl || !anonKey || !serviceRoleKey) {
    return jsonResponse(req, { error: "Lodge Mailroom is not configured" }, 503);
  }
  ...
  const { data: canManage } = await userClient.rpc("has_admin_section_permission", {
  ...
  try {
    ...
    if (!limit.allowed) {
      return jsonResponse(req, { error: "Mailroom request limit reached. Please try again later." }, 429, {
    ...
    if (action === "approve" || action === "reject") {
      ...
      if (!body.proposal || typeof body.proposal !== "object" || Array.isArray(body.proposal)) {
        return jsonResponse(req, { error: "Review the proposed items before publishing" }, 400);
      }
      const proposal = asObject(body.proposal);
      const publicationTarget = proposal.publication_target === "district"
        ? "district"
        : proposal.publication_target === "carleton" ? "carleton" : "";
      if (!publicationTarget) {
        return jsonResponse(req, { error: "Choose Carleton Lodge or Ottawa District 1 before publishing" }, 400);
      }
    ...
    const inboundEmailId = typeof body.inboundEmailId === "string" ? body.inboundEmailId : "";
    if (!UUID_PATTERN.test(inboundEmailId)) {
      return jsonResponse(req, { error: "A valid inbound email is required" }, 400);
    }
    ...
    if (!openAiKey || !emailApiKey) {
      return jsonResponse(req, { error: "Mailroom extraction is not configured yet" }, 503);
    }
    ...
    const { data: existingImport } = await adminClient
      ...
      .maybeSingle();
    if (existingImport?.status === "needs_review" || existingImport?.status === "approved") {
      return jsonResponse(req, { import: existingImport, reused: true });
    }
    if (existingImport?.status === "rejected") {
      return jsonResponse(req, { error: "This email was already rejected" }, 409);
    }
    ...
    if (!email) return jsonResponse(req, { error: "Inbound email not found" }, 404);
    ...
    if (!trustedSender) {
      return jsonResponse(req, {
        error: "Add this exact sender to the trusted-sender list before preparing a draft.",
      }, 409);
    }
    if (!messageAuthenticationPassed(email.headers)) {
      return jsonResponse(req, {
        error: "This message did not pass the required DMARC or DKIM/SPF authentication checks.",
      }, 409);
    }

    const importId = existingImport?.id ?? crypto.randomUUID();
    const { error: draftError } = await adminClient.from("mailroom_imports").upsert({
      id: importId,
    ...
    if (pdfAttachment) {
      ...
      sourceFile = {
        ...
        provider_attachment_id: attachmentValue(pdfAttachment, "id", "attachment_id", "attachmentId"),
      };
    }
    ...
    const htmlFallback = plainText
      ? ""
      : String(email.html_body ?? "").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").slice(0, 150_000);
    ...
    const instructions = `You extract proposed website updates for Carleton Lodge No. 465 in Carp, Ontario, including visiting-lodge summons from Ottawa District 1.
...
10. The result is only a draft. A human will review it before publication.`;
    ...
    if (!modelResponse.ok) {
      console.error("Mailroom extraction failed", modelResponse.status, responseJson);
    ...
    const message = error instanceof Error ? error.message : "Lodge Mailroom failed";
    if (action === "process" && typeof body.inboundEmailId === "string" && UUID_PATTERN.test(body.inboundEmailId)) {
