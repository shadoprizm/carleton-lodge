  ...
  try {
    ...
    if (!member) {
      return jsonResponse(req, {
        error: "No lodge member profile is linked to this account",
      }, 404);
    }
    ...
    let accountQuery = supabaseAdmin
      ...
      .select(
        "id, address, account_type, status, credential_status, agreement_required, activated_at",
      )
    ...
    if (!account) {
      return jsonResponse(req, {
        error: "Your personal Lodge mailbox is not available",
      }, 404);
    }
    if (
      account.status === "SUSPENDED" || member.mailbox_status === "suspended"
    ) {
      return jsonResponse(req, {
        error: "This mailbox is suspended. Please contact Lodge Support.",
      }, 409);
    }
    if (
      ["NOT_PROVISIONED", "PROVISIONING", "ERROR", "DISABLED"].includes(
        account.status,
      )
    ) {
      return jsonResponse(req, {
        error: "Your mailbox is not ready for activation yet",
      }, 409);
    }
    ...
    if (!policy) {
      return jsonResponse(req, {
        error: "The member email agreement is not currently available",
      }, 503);
    }

    const { data: currentAcceptance, error: acceptanceError } =
      await supabaseAdmin
    ...
    if (
      ...
      return jsonResponse(
        ...
        {
          error:
            "You must read and accept the current Member Email Account Agreement before activation.",
        },
        400,
      );
    }
    ...
    if (needsPasswordSetup) {
      ...
      if (passwordError) {
        return jsonResponse(req, { error: passwordError }, 400);
      }
      ...
      if (!providerAccount) {
        return jsonResponse(req, {
          error: "The MXroute mailbox does not exist yet",
        }, 409);
      }
    ...
    return jsonResponse(
      ...
      {
        error:
          "We could not activate the mailbox. Please try again or contact Lodge Support.",
      },
      500,
    );
  }
