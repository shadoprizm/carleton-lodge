...
function validatePassword(password: string) {
  ...
  if (/\r|\n/.test(password)) {
    return "The password cannot contain a line break.";
  }
  return "";
}
...
async function getRoleAccount(
  ...
  const { data, error } = await supabaseAdmin
    ...
    .select(
      "id, address, account_type, status, position_id, current_authorized_member_id, display_name, agreement_required, credential_status, provisioned_at",
    )
    .eq("id", accountId)
...
async function queueAccountEmail(
  ...
  input: {
    notificationType:
      ...
      | "email_account_password_reset"
      | "email_account_activation_confirmation";
  ...
  if (!input.recipient.email || !input.recipient.linked_profile_id) {
    throw new Error(
      "The member needs a verified personal email and linked website account first",
    );
  }
...
async function prepareRoleInvitation(
  ...
  if (!member.email || !member.linked_profile_id) {
    throw new Error(
      "The incoming holder needs a verified personal email and linked website account first",
    );
  }
  ...
  if (!providerAccount) {
    throw new Error("Provision the role mailbox before sending an invitation");
  }

  await provider.updateMailbox(account.address, {
    password: createProviderLockPassword(),
  });
  const now = new Date().toISOString();

  const { data: existingAssignment, error: assignmentReadError } =
    await supabaseAdmin
  ...
  if (existingAssignment) {
    ...
    const { error } = await supabaseAdmin
      ...
      .insert({
        ...
        assigned_by: actorId,
        reason: handoverId
          ? "Officer mailbox handover"
          : "Initial role mailbox activation",
      });
    if (error) throw error;
  }
...
async function completeAccountAction(
  ...
  if (rawToken.length < 32 || rawToken.length > 256) {
    return jsonResponse(
      req,
      { error: "This secure email link is invalid" },
      400,
    );
  }
  ...
  const { data: actionToken, error: tokenError } = await supabaseAdmin
    ...
    .select(
      "id, purpose, email_account_id, member_id, handover_id, expires_at, consumed_at, revoked_at",
    )
    .eq("token_hash", tokenHash)
    .maybeSingle();
  if (tokenError) throw tokenError;
  if (
    !actionToken || actionToken.member_id !== member.id ||
    actionToken.consumed_at || actionToken.revoked_at
  ) {
    return jsonResponse(req, {
      error: "This secure email link is invalid or has already been used",
    }, 400);
  }
  if (new Date(actionToken.expires_at).getTime() <= Date.now()) {
    return jsonResponse(req, {
      error: "This secure email link has expired. Request a new one.",
    }, 400);
  }
  ...
  const { data: account, error: accountError } = await supabaseAdmin
    .from("lodge_email_accounts")
    .select(
      "id, address, account_type, status, position_id, associated_member_id, current_authorized_member_id, agreement_required, activated_at",
    )
  ...
  if (!account) {
    return jsonResponse(
      req,
      { error: "The Lodge mailbox no longer exists" },
      404,
    );
  }
  ...
  if (!isPersonalOwner && !isCurrentRoleHolder) {
    return jsonResponse(req, {
      error: "You are no longer authorized for this mailbox",
    }, 403);
  }
  if (
    ["SUSPENDED", "DISABLED", "ERROR", "NOT_PROVISIONED", "PROVISIONING"]
      .includes(account.status)
  ) {
    return jsonResponse(req, {
      error: "This mailbox is not currently available",
    }, 409);
  }
  ...
  if (!policy) {
    return jsonResponse(req, {
      error: "The required email agreement is unavailable",
    }, 503);
  }

  const { data: currentAcceptance, error: acceptanceReadError } =
    await supabaseAdmin
  ...
  if (!acceptance && policy.requires_reacceptance === false) {
    const { data: priorAcceptance, error: priorAcceptanceError } =
      await supabaseAdmin
        .from("email_agreement_acceptances")
        .select("id")
        .eq("member_id", member.id)
  ...
  if (
    needsAgreement &&
    (body.agreementAccepted !== true || policyVersionId !== policy.id)
  ) {
    return jsonResponse(
      req,
      {
        error:
          "You must read and accept the current email agreement before continuing.",
      },
      400,
    );
  }
  ...
  if (account.account_type !== "MEMBER") {
    const { data: assignment, error: assignmentError } = await supabaseAdmin
      ...
      .in(
        "status",
        actionToken.purpose === "ROLE_ACTIVATION" ? ["PENDING"] : ["ACTIVE"],
      )
      .maybeSingle();
    if (assignmentError) throw assignmentError;
    if (!assignment) {
      return jsonResponse(req, {
        error: "Your role-mailbox assignment is no longer current",
      }, 403);
    }
  }
  ...
  try {
    ...
    if (
      ...
      providerError.message === "The MXroute mailbox does not exist"
    ) {
  ...
  const now = new Date().toISOString();
  if (needsAgreement) {
    const { error } = await supabaseAdmin.from("email_agreement_acceptances")
      .insert({
        member_id: member.id,
        email_account_id: account.id,
        position_id: account.position_id,
  ...
  const { error: accountUpdateError } = await supabaseAdmin
    ...
    .update({
      ...
      last_credential_rotation_at: now,
      provider_status: latestProvider
        ? mailboxProviderStatusJson(latestProvider)
        : {},
      updated_at: now,
    })
    .eq("id", account.id);
  ...
  try {
    ...
    const limit = await consumeRateLimit(
      ...
      action.startsWith("admin_")
        ? "lodge-email-admin:user"
        : "lodge-email-member:user",
      user.id,
    ...
    if (!limit.allowed) {
      return jsonResponse(req, {
        error: "Too many email-account requests. Please wait and try again.",
      }, 429);
    }
    ...
    if (action === "complete_account_action") {
      if (!currentMember) {
        return jsonResponse(req, {
          error: "No verified Lodge member is linked to this account",
        }, 403);
      }
    ...
    if (action === "request_password_reset") {
      ...
      const { data: account, error } = await supabaseAdmin
        .from("lodge_email_accounts")
        .select(
          "id, address, account_type, display_name, position_id, associated_member_id, current_authorized_member_id, status",
        )
        .eq("id", accountId)
        .maybeSingle();
      if (error) throw error;
      if (!account) {
        return jsonResponse(req, { error: "Mailbox not found" }, 404);
      }
      ...
      if ((!personalOwner && !roleHolder) || account.status !== "ACTIVE") {
        return jsonResponse(req, {
          error: "You are not authorized to reset this mailbox",
        }, 403);
      }
      if (!currentMember.email || !currentMember.linked_profile_id) {
        return jsonResponse(req, {
          error: "A verified personal email is required",
        }, 409);
      }
      ...
      const notification = await queueAccountEmail(supabaseAdmin, {
        ...
        account,
        idempotencyKey:
          `email-account-password-reset:${account.id}:${requestId}`,
    ...
    if (
      !action.startsWith("admin_") || !await requireMemberWrite(supabaseUser)
    ) {
      return jsonResponse(req, { error: "Forbidden" }, 403);
    }

    if (action === "admin_create_role_account") {
      const positionId = typeof body?.positionId === "string"
        ? body.positionId
        : "";
      const displayName = typeof body?.displayName === "string"
        ? body.displayName.trim().slice(0, 160)
        : "";
      const accountType = body?.accountType === "OFFICER"
        ? "OFFICER"
        : "FUNCTIONAL";
      const incomingMemberId = typeof body?.incomingMemberId === "string" &&
          uuidPattern.test(body.incomingMemberId)
        ? body.incomingMemberId
        : null;
      if (!uuidPattern.test(positionId) || !displayName) {
        return jsonResponse(req, {
          error: "A Lodge position and display name are required",
        }, 400);
      }
      let address = "";
      try {
        ...
        return jsonResponse(req, {
          error: error instanceof Error
            ? error.message
            : "A valid Lodge email is required",
        }, 400);
      }
      if (incomingMemberId) {
        ...
        if (!holder) {
          return jsonResponse(req, {
            error: "The selected member was not found",
          }, 404);
        }
      }
    ...
    if (action === "admin_create_policy_version") {
      ...
      const effectiveAt = typeof body?.effectiveAt === "string"
        ...
        : new Date();
      if (
        !policyType || title.length < 5 || content.length < 100 ||
        acknowledgement.length < 40 || Number.isNaN(effectiveAt.getTime())
      ) {
        return jsonResponse(req, {
          error:
            "A complete policy, acknowledgement, and valid effective date are required",
        }, 400);
      }
    ...
    if (
      ...
      const { data: personalAccount, error: personalAccountError } =
        await supabaseAdmin
          .from("lodge_email_accounts")
          .select(
            "id, address, account_type, status, associated_member_id, display_name",
          )
          .eq("id", accountId)
      ...
      if (!personalAccount?.associated_member_id) {
        return jsonResponse(
          req,
          { error: "Personal Lodge mailbox not found" },
          404,
        );
      }
      const member = await getMember(
        supabaseAdmin,
        personalAccount.associated_member_id,
      );
      if (!member) {
        return jsonResponse(req, { error: "Mailbox member not found" }, 404);
      }
      ...
      if (action === "admin_suspend_personal_account") {
        ...
        const provider = createMxrouteProvider();
        await provider.updateMailbox(personalAccount.address, {
          password: createProviderLockPassword(),
        });
        const now = new Date().toISOString();
        const { error } = await supabaseAdmin
      ...
      if (
        action === "admin_initiate_personal_password_reset" &&
        personalAccount.status !== "ACTIVE"
      ) {
        return jsonResponse(req, {
          error: "Only an active mailbox can receive a standard password reset",
        }, 409);
      }
      ...
      const notification = await queueAccountEmail(supabaseAdmin, {
        ...
        idempotencyKey:
          `${action}:${personalAccount.id}:${crypto.randomUUID()}`,
        purpose: "PASSWORD_RESET",
      });
    ...
    if (!account) {
      return jsonResponse(req, { error: "Role mailbox not found" }, 404);
    }
    ...
    if (action === "admin_send_role_invitation") {
      if (!account.current_authorized_member_id) {
        return jsonResponse(req, {
          error: "Assign a member to this role before sending an invitation",
        }, 409);
      }
      const member = await getMember(
        supabaseAdmin,
      ...
      if (!member) {
        return jsonResponse(req, { error: "Assigned member not found" }, 404);
      }
      const requestId =
        typeof body?.requestId === "string" && uuidPattern.test(body.requestId)
          ? body.requestId
    ...
    if (action === "admin_initiate_password_reset") {
      if (
        !account.current_authorized_member_id || account.status !== "ACTIVE"
      ) {
        return jsonResponse(req, {
          error: "Only an active current holder can receive a password reset",
        }, 409);
      }
      const member = await getMember(
        supabaseAdmin,
      ...
      const notification = await queueAccountEmail(supabaseAdmin, {
        ...
        account,
        idempotencyKey:
          `admin-email-account-password-reset:${account.id}:${crypto.randomUUID()}`,
        purpose: "PASSWORD_RESET",
      });
      await insertAudit(supabaseAdmin, {
    ...
    if (action === "admin_update_role_configuration") {
      ...
      if (!enabled && account.current_authorized_member_id) {
        return jsonResponse(req, {
          error:
            "Vacate or suspend the assigned role before disabling its configuration",
        }, 409);
      }
      const { error } = await supabaseAdmin
        .from("lodge_email_accounts")
    ...
    if (action === "admin_initiate_handover") {
      if (body?.confirmed !== true) {
        return jsonResponse(req, {
          error: "Explicit handover confirmation is required",
        }, 400);
      }
      const incomingMemberId = typeof body?.incomingMemberId === "string"
        ? body.incomingMemberId
      ...
      if (!uuidPattern.test(incomingMemberId)) {
        return jsonResponse(req, {
          error: "A valid incoming member is required",
        }, 400);
      }
      if (incomingMemberId === account.current_authorized_member_id) {
        return jsonResponse(req, {
          error: "The incoming member already holds this role mailbox",
        }, 409);
      }
      ...
      if (!incomingMember?.email || !incomingMember.linked_profile_id) {
        return jsonResponse(
          ...
          {
            error:
              "The incoming member needs a verified personal email and linked website account first",
          },
          409,
        );
      }
      ...
      try {
        ...
        return jsonResponse(req, {
          error:
            "The handover needs administrator attention. Website access was revoked, but MXroute credential rotation or the incoming invitation failed.",
          handoverId: handover.id,
          state: "FAILED",
    ...
    if (action === "admin_retry_handover") {
      ...
      if (!handover?.incoming_member_id) {
        return jsonResponse(req, {
          error: "No failed handover is available to retry",
        }, 409);
      }
      const incomingMember = await getMember(
        supabaseAdmin,
        handover.incoming_member_id,
      );
      if (!incomingMember) {
        return jsonResponse(req, { error: "Incoming member not found" }, 404);
      }
      await supabaseAdmin
        .from("officer_email_handovers")
        .update({
      ...
      try {
        ...
        const failure = retryError instanceof Error
          ? retryError.message
        ...
        await supabaseAdmin
          ...
          .eq("id", handover.id);
        return jsonResponse(req, {
          error: "The handover retry failed",
          handoverId: handover.id,
        }, 502);
      }
    }
    ...
    if (action === "admin_reactivate_account") {
      if (!account.current_authorized_member_id) {
        return jsonResponse(req, {
          error: "Assign a current holder before reactivation",
        }, 409);
      }
      const member = await getMember(
        supabaseAdmin,
    ...
    return jsonResponse(
      req,
      {
        error: error instanceof Error
          ? error.message
          : "The Lodge email request failed",
      },
      500,
    );
  }
