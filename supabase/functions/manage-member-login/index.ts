...
async function reserveLodgeEmail(
  ...
  for (let suffix = 1; suffix <= 100; suffix += 1) {
    ...
    const [{ data: existing, error }, { data: governedAccount, error: accountError }] = await Promise.all([
      supabaseAdmin
      .from("lodge_members")
      .select("id")
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: member, error: memberError } = await supabaseAdmin
      .from("lodge_members")
      .select("id, full_name, linked_profile_id, email, lodge_email, mailbox_status, mailbox_quota_mb, mailbox_send_limit")
      .eq("id", memberId)
      .maybeSingle();
    ...
    const governedAccountValues = {
      ...
      credential_status: mailboxAlreadyActive ? "USER_SET" : "PROVISIONED_LOCKED",
    ...
    if (governedAccountId) {
      ...
      const { data: createdGovernedAccount, error: governedAccountInsertError } =
        await supabaseAdmin
          .from("lodge_email_accounts")
