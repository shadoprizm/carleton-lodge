...
CREATE TABLE carletonlodge.district_lodges (
...
CREATE TABLE carletonlodge.district_summons (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  lodge_id uuid NOT NULL REFERENCES carletonlodge.district_lodges(id) ON DELETE RESTRICT,
  title text NOT NULL,
...
CREATE TABLE carletonlodge.district_events (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  lodge_id uuid NOT NULL REFERENCES carletonlodge.district_lodges(id) ON DELETE RESTRICT,
  summons_id uuid NOT NULL REFERENCES carletonlodge.district_summons(id) ON DELETE CASCADE,
  title text NOT NULL,
  description text,
...
ALTER TABLE carletonlodge.mailroom_imports
  ADD COLUMN published_district_summons_id uuid
    REFERENCES carletonlodge.district_summons(id) ON DELETE SET NULL,
  ADD COLUMN published_district_event_ids uuid[] NOT NULL DEFAULT '{}';

CREATE INDEX district_summons_lodge_published_idx
  ON carletonlodge.district_summons(lodge_id, published_at DESC);
CREATE INDEX district_summons_published_by_idx
  ON carletonlodge.district_summons(published_by)
  WHERE published_by IS NOT NULL;
CREATE INDEX district_events_upcoming_idx
  ON carletonlodge.district_events(event_date, event_time, lodge_id);
CREATE INDEX district_events_lodge_idx
  ON carletonlodge.district_events(lodge_id, event_date DESC);
CREATE INDEX district_events_summons_idx
  ON carletonlodge.district_events(summons_id);
CREATE INDEX mailroom_imports_published_district_summons_idx
  ON carletonlodge.mailroom_imports(published_district_summons_id)
  WHERE published_district_summons_id IS NOT NULL;

ALTER TABLE carletonlodge.district_lodges ENABLE ROW LEVEL SECURITY;
ALTER TABLE carletonlodge.district_summons ENABLE ROW LEVEL SECURITY;
ALTER TABLE carletonlodge.district_events ENABLE ROW LEVEL SECURITY;

GRANT SELECT, INSERT, UPDATE, DELETE ON carletonlodge.district_lodges TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON carletonlodge.district_summons TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON carletonlodge.district_events TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON carletonlodge.district_lodges TO service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON carletonlodge.district_summons TO service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON carletonlodge.district_events TO service_role;

CREATE POLICY "Members can view District 1 lodges"
  ON carletonlodge.district_lodges FOR SELECT TO authenticated
  USING (EXISTS (
    SELECT 1 FROM carletonlodge.profiles
    WHERE profiles.id = (SELECT auth.uid())
  ));
CREATE POLICY "Members can view District 1 summons"
  ON carletonlodge.district_summons FOR SELECT TO authenticated
  USING (EXISTS (
    SELECT 1 FROM carletonlodge.profiles
    WHERE profiles.id = (SELECT auth.uid())
  ));
CREATE POLICY "Members can view District 1 events"
  ON carletonlodge.district_events FOR SELECT TO authenticated
  USING (EXISTS (
    SELECT 1 FROM carletonlodge.profiles
    WHERE profiles.id = (SELECT auth.uid())
  ));
...
CREATE POLICY "Communications writers can create District 1 lodges"
  ON carletonlodge.district_lodges FOR INSERT TO authenticated
...
CREATE POLICY "Communications writers can update District 1 lodges"
  ON carletonlodge.district_lodges FOR UPDATE TO authenticated
...
CREATE POLICY "Communications writers can delete District 1 lodges"
  ON carletonlodge.district_lodges FOR DELETE TO authenticated
  USING (carletonlodge.has_admin_section_permission('communications', 'write'));

CREATE POLICY "Communications writers can create District 1 summons"
  ON carletonlodge.district_summons FOR INSERT TO authenticated
  WITH CHECK (
...
CREATE POLICY "Communications writers can update District 1 summons"
  ON carletonlodge.district_summons FOR UPDATE TO authenticated
  USING (carletonlodge.has_admin_section_permission('communications', 'write'))
  WITH CHECK (carletonlodge.has_admin_section_permission('communications', 'write'));
CREATE POLICY "Communications writers can delete District 1 summons"
  ON carletonlodge.district_summons FOR DELETE TO authenticated
  USING (carletonlodge.has_admin_section_permission('communications', 'write'));

CREATE POLICY "Communications writers can create District 1 events"
  ON carletonlodge.district_events FOR INSERT TO authenticated
  WITH CHECK (carletonlodge.has_admin_section_permission('communications', 'write'));
CREATE POLICY "Communications writers can update District 1 events"
  ON carletonlodge.district_events FOR UPDATE TO authenticated
  USING (carletonlodge.has_admin_section_permission('communications', 'write'))
  WITH CHECK (carletonlodge.has_admin_section_permission('communications', 'write'));
CREATE POLICY "Communications writers can delete District 1 events"
  ON carletonlodge.district_events FOR DELETE TO authenticated
  USING (carletonlodge.has_admin_section_permission('communications', 'write'));

CREATE TRIGGER update_district_lodges_updated_at
  BEFORE UPDATE ON carletonlodge.district_lodges
  FOR EACH ROW EXECUTE FUNCTION carletonlodge.update_updated_at_column();
CREATE TRIGGER update_district_summons_updated_at
  BEFORE UPDATE ON carletonlodge.district_summons
  FOR EACH ROW EXECUTE FUNCTION carletonlodge.update_updated_at_column();
CREATE TRIGGER update_district_events_updated_at
  BEFORE UPDATE ON carletonlodge.district_events
  FOR EACH ROW EXECUTE FUNCTION carletonlodge.update_updated_at_column();

ALTER TABLE carletonlodge.lodge_knowledge
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    DELETE FROM carletonlodge.lodge_knowledge
    WHERE source_type = 'district_lodge' AND source_id = OLD.id;
    RETURN OLD;
  END IF;
  INSERT INTO carletonlodge.lodge_knowledge (
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    ...
    RETURN OLD;
  END IF;
  SELECT name INTO lodge_name FROM carletonlodge.district_lodges WHERE id = NEW.lodge_id;
  INSERT INTO carletonlodge.lodge_knowledge (
    source_type, source_id, title, body, keywords, source_url, visibility, source_updated_at
  ) VALUES (
...
CREATE TRIGGER sync_district_lodge_knowledge
  AFTER INSERT OR UPDATE OR DELETE ON carletonlodge.district_lodges
...
CREATE TRIGGER sync_district_summons_knowledge
  AFTER INSERT OR UPDATE OR DELETE ON carletonlodge.district_summons
...
CREATE TRIGGER sync_district_event_knowledge
  AFTER INSERT OR UPDATE OR DELETE ON carletonlodge.district_events
  FOR EACH ROW EXECUTE FUNCTION carletonlodge_private.sync_district_event_knowledge();

CREATE OR REPLACE FUNCTION carletonlodge.approve_district_mailroom_import(
...
BEGIN
  ...
  FROM carletonlodge.district_lodges
  ...
  IF target_lodge_id IS NULL THEN
    ...
    IF EXISTS (SELECT 1 FROM carletonlodge.district_lodges WHERE slug = lodge_slug) THEN
      lodge_slug := lodge_slug || '-' || substr(replace(gen_random_uuid()::text, '-', ''), 1, 8);
    END IF;
    INSERT INTO carletonlodge.district_lodges (
  ...
  ELSE
    UPDATE carletonlodge.district_lodges SET
  ...
  INSERT INTO carletonlodge.district_summons (
  ...
  LOOP
    ...
    INSERT INTO carletonlodge.district_events (
  ...
  UPDATE carletonlodge.mailroom_imports SET
  ...
  UPDATE carletonlodge.inbound_emails SET
...
REVOKE ALL ON FUNCTION carletonlodge.approve_district_mailroom_import(uuid, jsonb)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION carletonlodge.approve_district_mailroom_import(uuid, jsonb)
  TO authenticated;
