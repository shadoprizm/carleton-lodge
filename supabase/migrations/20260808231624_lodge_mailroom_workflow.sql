...
CREATE TABLE IF NOT EXISTS public.trusted_email_senders (
...
ALTER TABLE public.trusted_email_senders ENABLE ROW LEVEL SECURITY;

CREATE INDEX IF NOT EXISTS trusted_email_senders_active_idx
  ON public.trusted_email_senders(is_active, email);

DROP TRIGGER IF EXISTS update_trusted_email_senders_updated_at
  ON public.trusted_email_senders;
CREATE TRIGGER update_trusted_email_senders_updated_at
  BEFORE UPDATE ON public.trusted_email_senders
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

GRANT SELECT, INSERT, UPDATE, DELETE
  ON public.trusted_email_senders TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE
  ON public.trusted_email_senders TO service_role;

CREATE POLICY "Communications readers can view trusted senders"
  ON public.trusted_email_senders FOR SELECT
  TO authenticated
...
CREATE POLICY "Communications writers can add trusted senders"
  ON public.trusted_email_senders FOR INSERT
  TO authenticated
  WITH CHECK (
...
CREATE POLICY "Communications writers can update trusted senders"
  ON public.trusted_email_senders FOR UPDATE
  TO authenticated
...
CREATE POLICY "Communications writers can delete trusted senders"
  ON public.trusted_email_senders FOR DELETE
  TO authenticated
  USING (public.has_admin_section_permission('communications', 'write'));

CREATE TABLE IF NOT EXISTS public.mailroom_imports (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  published_summons_id uuid REFERENCES public.summons(id) ON DELETE SET NULL,
...
ALTER TABLE public.mailroom_imports ENABLE ROW LEVEL SECURITY;

CREATE INDEX IF NOT EXISTS mailroom_imports_review_queue_idx
  ON public.mailroom_imports(status, created_at DESC);

DROP TRIGGER IF EXISTS update_mailroom_imports_updated_at
  ON public.mailroom_imports;
CREATE TRIGGER update_mailroom_imports_updated_at
  BEFORE UPDATE ON public.mailroom_imports
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

GRANT SELECT ON public.mailroom_imports TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.mailroom_imports TO service_role;

CREATE POLICY "Communications readers can view mailroom imports"
  ON public.mailroom_imports FOR SELECT
  TO authenticated
  USING (public.has_admin_section_permission('communications', 'read'));
...
CREATE UNIQUE INDEX IF NOT EXISTS summons_source_mailroom_import_unique
  ON public.summons(source_mailroom_import_id)
...
CREATE INDEX IF NOT EXISTS events_source_mailroom_import_idx
  ON public.events(source_mailroom_import_id)
  WHERE source_mailroom_import_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS announcements_source_mailroom_import_idx
  ON public.announcements(source_mailroom_import_id)
  WHERE source_mailroom_import_id IS NOT NULL;
...
CREATE OR REPLACE FUNCTION public.approve_mailroom_import(
...
BEGIN
  ...
  END IF;
  IF NOT public.has_admin_section_permission('communications', 'write') THEN
  ...
  UPDATE public.mailroom_imports
  SET status = 'approved',
...
CREATE OR REPLACE FUNCTION public.reject_mailroom_import(
  target_import_id uuid
)
RETURNS void
LANGUAGE plpgsql
