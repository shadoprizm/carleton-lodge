...
CREATE TABLE IF NOT EXISTS carletonlodge.announcements (
...
ALTER TABLE carletonlodge.announcements ENABLE ROW LEVEL SECURITY;

CREATE INDEX IF NOT EXISTS announcements_active_idx
  ON carletonlodge.announcements(is_published, visibility, published_at DESC)
  WHERE is_published = true;

DROP TRIGGER IF EXISTS update_announcements_updated_at
  ON carletonlodge.announcements;
CREATE TRIGGER update_announcements_updated_at
  BEFORE UPDATE ON carletonlodge.announcements
  FOR EACH ROW EXECUTE FUNCTION carletonlodge.update_updated_at_column();

GRANT SELECT ON carletonlodge.announcements TO anon;
GRANT SELECT, INSERT, UPDATE, DELETE ON carletonlodge.announcements TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON carletonlodge.announcements TO service_role;
...
CREATE POLICY "Public can view active public announcements"
  ON carletonlodge.announcements FOR SELECT
  TO anon
  USING (
    is_published = true
...
CREATE POLICY "Communications writers can create announcements"
  ON carletonlodge.announcements FOR INSERT
  TO authenticated
  WITH CHECK (
    created_by = (SELECT auth.uid())
    AND carletonlodge.has_admin_section_permission('communications', 'write')
  );
...
CREATE POLICY "Communications writers can update announcements"
  ON carletonlodge.announcements FOR UPDATE
  TO authenticated
...
CREATE POLICY "Communications writers can delete announcements"
  ON carletonlodge.announcements FOR DELETE
  TO authenticated
  USING (carletonlodge.has_admin_section_permission('communications', 'write'));

ALTER TABLE carletonlodge.notification_preferences
...
CREATE TRIGGER enqueue_announcement_notifications
  AFTER INSERT OR UPDATE OF is_published ON carletonlodge.announcements
