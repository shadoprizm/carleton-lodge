...
REVOKE SELECT ON public.lodge_members FROM authenticated;
GRANT SELECT (
  id,
  full_name,
  phone,
  ...
  created_at,
  updated_at
) ON public.lodge_members TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.lodge_members TO service_role;
REVOKE ALL ON public.lodge_members FROM anon;

CREATE OR REPLACE FUNCTION public.get_managed_lodge_members()
...
AS $$
BEGIN
  IF NOT public.has_admin_section_permission('members', 'read') THEN
