...
CREATE TABLE IF NOT EXISTS carletonlodge.help_topics (
...
ALTER TABLE carletonlodge.help_topics ENABLE ROW LEVEL SECURITY;

DROP TRIGGER IF EXISTS update_help_topics_updated_at ON carletonlodge.help_topics;
CREATE TRIGGER update_help_topics_updated_at
  BEFORE UPDATE ON carletonlodge.help_topics
  FOR EACH ROW EXECUTE FUNCTION carletonlodge.update_updated_at_column();

GRANT SELECT ON carletonlodge.help_topics TO anon;
GRANT SELECT, INSERT, UPDATE, DELETE ON carletonlodge.help_topics TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON carletonlodge.help_topics TO service_role;

CREATE POLICY "Public can view public help topics"
  ON carletonlodge.help_topics FOR SELECT TO anon
  USING (visibility = 'public');

CREATE POLICY "Members can view help topics"
  ON carletonlodge.help_topics FOR SELECT TO authenticated
  USING (visibility IN ('public', 'members'));

CREATE POLICY "Library editors can create help topics"
  ON carletonlodge.help_topics FOR INSERT TO authenticated
  WITH CHECK (carletonlodge.has_admin_section_permission('library', 'write'));

CREATE POLICY "Library editors can update help topics"
  ON carletonlodge.help_topics FOR UPDATE TO authenticated
  USING (carletonlodge.has_admin_section_permission('library', 'write'))
  WITH CHECK (carletonlodge.has_admin_section_permission('library', 'write'));

CREATE POLICY "Library editors can delete help topics"
  ON carletonlodge.help_topics FOR DELETE TO authenticated
  USING (carletonlodge.has_admin_section_permission('library', 'write'));

INSERT INTO carletonlodge.help_topics (category, title, body, keywords, url, visibility, display_order)
VALUES
...
CREATE TABLE IF NOT EXISTS carletonlodge.lodge_knowledge (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
ALTER TABLE carletonlodge.lodge_knowledge ENABLE ROW LEVEL SECURITY;

CREATE INDEX IF NOT EXISTS lodge_knowledge_search_idx
  ON carletonlodge.lodge_knowledge USING gin(search_vector);
CREATE INDEX IF NOT EXISTS lodge_knowledge_source_idx
  ON carletonlodge.lodge_knowledge(source_type, source_updated_at DESC);

GRANT SELECT ON carletonlodge.lodge_knowledge TO anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON carletonlodge.lodge_knowledge TO service_role;

CREATE POLICY "Public can search public lodge knowledge"
  ON carletonlodge.lodge_knowledge FOR SELECT TO anon
  USING (
    visibility = 'public'
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    ...
    RETURN OLD;
  END IF;
  INSERT INTO carletonlodge.lodge_knowledge (source_type, source_id, title, body, keywords, source_url, visibility, source_updated_at)
  VALUES (
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    DELETE FROM carletonlodge.lodge_knowledge WHERE source_type = 'announcement' AND source_id = OLD.id;
    RETURN OLD;
  END IF;
  IF NEW.is_published = false THEN
    DELETE FROM carletonlodge.lodge_knowledge WHERE source_type = 'announcement' AND source_id = NEW.id;
    RETURN NEW;
  END IF;
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    DELETE FROM carletonlodge.lodge_knowledge WHERE source_type = 'summons' AND source_id = OLD.id;
    RETURN OLD;
  END IF;
  INSERT INTO carletonlodge.lodge_knowledge (source_type, source_id, title, body, keywords, source_url, visibility, source_updated_at)
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    DELETE FROM carletonlodge.lodge_knowledge WHERE source_type = 'document' AND source_id = OLD.id;
    RETURN OLD;
  END IF;
  INSERT INTO carletonlodge.lodge_knowledge (source_type, source_id, title, body, keywords, source_url, visibility, source_updated_at)
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    DELETE FROM carletonlodge.lodge_knowledge WHERE source_type = 'history' AND source_id = OLD.id;
    RETURN OLD;
  END IF;
  INSERT INTO carletonlodge.lodge_knowledge (source_type, source_id, title, body, keywords, source_url, visibility, source_updated_at)
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    DELETE FROM carletonlodge.lodge_knowledge WHERE source_type = 'member' AND source_id = OLD.id;
    RETURN OLD;
  END IF;
  SELECT name INTO position_name FROM carletonlodge.lodge_positions WHERE id = NEW.position_id;
  INSERT INTO carletonlodge.lodge_knowledge (source_type, source_id, title, body, keywords, source_url, visibility, source_updated_at)
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    DELETE FROM carletonlodge.lodge_knowledge WHERE source_type = 'help' AND source_id = OLD.id;
    RETURN OLD;
  END IF;
  INSERT INTO carletonlodge.lodge_knowledge (source_type, source_id, title, body, keywords, source_url, visibility, source_updated_at)
...
CREATE TRIGGER sync_event_knowledge AFTER INSERT OR UPDATE OR DELETE ON carletonlodge.events FOR EACH ROW EXECUTE FUNCTION carletonlodge_private.sync_event_knowledge();
CREATE TRIGGER sync_announcement_knowledge AFTER INSERT OR UPDATE OR DELETE ON carletonlodge.announcements FOR EACH ROW EXECUTE FUNCTION carletonlodge_private.sync_announcement_knowledge();
CREATE TRIGGER sync_summons_knowledge AFTER INSERT OR UPDATE OR DELETE ON carletonlodge.summons FOR EACH ROW EXECUTE FUNCTION carletonlodge_private.sync_summons_knowledge();
CREATE TRIGGER sync_document_knowledge AFTER INSERT OR UPDATE OR DELETE ON carletonlodge.documents FOR EACH ROW EXECUTE FUNCTION carletonlodge_private.sync_document_knowledge();
CREATE TRIGGER sync_history_era_knowledge AFTER INSERT OR UPDATE OR DELETE ON carletonlodge.history_eras FOR EACH ROW EXECUTE FUNCTION carletonlodge_private.sync_history_era_knowledge();
CREATE TRIGGER sync_member_knowledge AFTER INSERT OR UPDATE OR DELETE ON carletonlodge.lodge_members FOR EACH ROW EXECUTE FUNCTION carletonlodge_private.sync_member_knowledge();
CREATE TRIGGER sync_help_knowledge AFTER INSERT OR UPDATE OR DELETE ON carletonlodge.help_topics FOR EACH ROW EXECUTE FUNCTION carletonlodge_private.sync_help_knowledge();

-- Backfill approved current sources.
INSERT INTO carletonlodge.lodge_knowledge (source_type, source_id, title, body, keywords, source_url, visibility, source_updated_at)
SELECT 'event', id, title, carletonlodge_private.knowledge_plain_text(concat_ws(' ', description, event_date::text, event_time::text, location, location_address, event_status, status_note)), 'calendar meeting event date time location cancellation postponement', '/calendar', visibility, updated_at FROM carletonlodge.events
ON CONFLICT (source_type, source_id) DO NOTHING;

INSERT INTO carletonlodge.lodge_knowledge (source_type, source_id, title, body, keywords, source_url, visibility, valid_until, source_updated_at)
SELECT 'announcement', id, title, carletonlodge_private.knowledge_plain_text(body), priority || ' notice announcement', CASE WHEN visibility = 'members' THEN '/my-lodge' ELSE '/' END, visibility, expires_at, updated_at FROM carletonlodge.announcements WHERE is_published = true
ON CONFLICT (source_type, source_id) DO NOTHING;

INSERT INTO carletonlodge.lodge_knowledge (source_type, source_id, title, body, keywords, source_url, visibility, source_updated_at)
SELECT 'summons', id, title, carletonlodge_private.knowledge_plain_text(month || ' ' || content), 'summons monthly notice agenda', '/summons', 'members', updated_at FROM carletonlodge.summons
ON CONFLICT (source_type, source_id) DO NOTHING;

INSERT INTO carletonlodge.lodge_knowledge (source_type, source_id, title, body, keywords, source_url, visibility, source_updated_at)
SELECT 'document', id, title, carletonlodge_private.knowledge_plain_text(concat_ws(' ', description, file_name)), array_to_string(tags, ' '), '/library', 'members', updated_at FROM carletonlodge.documents
ON CONFLICT (source_type, source_id) DO NOTHING;

INSERT INTO carletonlodge.lodge_knowledge (source_type, source_id, title, body, keywords, source_url, visibility, source_updated_at)
SELECT 'history', id, title, carletonlodge_private.knowledge_plain_text(concat_ws(' ', year_start::text, year_end::text, summary, content)), 'history heritage timeline', '/history/' || slug, 'public', updated_at FROM carletonlodge.history_eras
ON CONFLICT (source_type, source_id) DO NOTHING;
...
LEFT JOIN carletonlodge.lodge_positions AS position ON position.id = member.position_id
ON CONFLICT (source_type, source_id) DO NOTHING;

INSERT INTO carletonlodge.lodge_knowledge (source_type, source_id, title, body, keywords, source_url, visibility, source_updated_at)
SELECT 'help', id, title, carletonlodge_private.knowledge_plain_text(body), array_to_string(keywords, ' '), url, visibility, updated_at FROM carletonlodge.help_topics
ON CONFLICT (source_type, source_id) DO NOTHING;
