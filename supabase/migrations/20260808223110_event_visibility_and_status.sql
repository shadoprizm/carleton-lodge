...
CREATE INDEX IF NOT EXISTS events_visibility_date_idx
  ON carletonlodge.events(visibility, event_date);

ALTER TABLE carletonlodge.event_submissions
...
DROP POLICY IF EXISTS "Anyone can view events" ON carletonlodge.events;
DROP POLICY IF EXISTS "Public can view public events" ON carletonlodge.events;
...
CREATE POLICY "Public can view public events"
  ON carletonlodge.events FOR SELECT
  TO anon
...
CREATE POLICY "Event managers can view all events"
  ON carletonlodge.events FOR SELECT
  TO authenticated
  USING (carletonlodge.has_admin_section_permission('events', 'read'));
