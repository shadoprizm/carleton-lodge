...
CREATE TABLE public.lodge_email_accounts (
  ...
  associated_member_id uuid REFERENCES public.lodge_members(id) ON DELETE RESTRICT,
  position_id uuid REFERENCES public.lodge_positions(id) ON DELETE RESTRICT,
  current_authorized_member_id uuid REFERENCES public.lodge_members(id) ON DELETE SET NULL,
...
CREATE UNIQUE INDEX email_policy_versions_one_active_idx
  ON public.email_policy_versions (policy_type)
  WHERE is_active;

CREATE TABLE public.officer_email_handovers (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  email_account_id uuid NOT NULL REFERENCES public.lodge_email_accounts(id) ON DELETE RESTRICT,
  position_id uuid NOT NULL REFERENCES public.lodge_positions(id) ON DELETE RESTRICT,
  outgoing_member_id uuid REFERENCES public.lodge_members(id) ON DELETE SET NULL,
  incoming_member_id uuid REFERENCES public.lodge_members(id) ON DELETE SET NULL,
...
CREATE UNIQUE INDEX officer_email_handovers_one_open_idx
  ON public.officer_email_handovers (email_account_id)
  WHERE state NOT IN ('ACTIVE', 'CANCELLED');
CREATE INDEX officer_email_handovers_state_idx
  ON public.officer_email_handovers (state, initiated_at DESC);

CREATE TABLE public.officer_mailbox_assignments (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  email_account_id uuid NOT NULL REFERENCES public.lodge_email_accounts(id) ON DELETE RESTRICT,
  position_id uuid NOT NULL REFERENCES public.lodge_positions(id) ON DELETE RESTRICT,
  member_id uuid NOT NULL REFERENCES public.lodge_members(id) ON DELETE RESTRICT,
  ...
  handover_id uuid REFERENCES public.officer_email_handovers(id) ON DELETE SET NULL,
...
CREATE UNIQUE INDEX officer_mailbox_assignments_one_current_idx
  ON public.officer_mailbox_assignments (email_account_id)
  WHERE status IN ('PENDING', 'ACTIVE');
CREATE INDEX officer_mailbox_assignments_member_idx
  ON public.officer_mailbox_assignments (member_id, status);
CREATE INDEX officer_mailbox_assignments_history_idx
  ON public.officer_mailbox_assignments (email_account_id, assignment_start DESC);

CREATE TABLE public.email_agreement_acceptances (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  member_id uuid NOT NULL REFERENCES public.lodge_members(id) ON DELETE RESTRICT,
  email_account_id uuid NOT NULL REFERENCES public.lodge_email_accounts(id) ON DELETE RESTRICT,
  position_id uuid REFERENCES public.lodge_positions(id) ON DELETE RESTRICT,
  policy_version_id uuid NOT NULL REFERENCES public.email_policy_versions(id) ON DELETE RESTRICT,
...
CREATE INDEX email_agreement_acceptances_account_idx
  ON public.email_agreement_acceptances (email_account_id, accepted_at DESC);
CREATE INDEX email_agreement_acceptances_member_idx
  ON public.email_agreement_acceptances (member_id, accepted_at DESC);

CREATE TABLE public.email_account_action_tokens (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  token_hash text NOT NULL UNIQUE CHECK (token_hash ~ '^[0-9a-f]{64}$'),
  purpose text NOT NULL CHECK (
    purpose IN ('ROLE_ACTIVATION', 'PASSWORD_RESET')
  ),
  email_account_id uuid NOT NULL REFERENCES public.lodge_email_accounts(id) ON DELETE CASCADE,
  member_id uuid NOT NULL REFERENCES public.lodge_members(id) ON DELETE CASCADE,
  handover_id uuid REFERENCES public.officer_email_handovers(id) ON DELETE CASCADE,
...
CREATE INDEX email_account_action_tokens_lookup_idx
  ON public.email_account_action_tokens (token_hash)
  WHERE consumed_at IS NULL AND revoked_at IS NULL;
CREATE INDEX email_account_action_tokens_account_idx
  ON public.email_account_action_tokens (email_account_id, purpose, created_at DESC);

CREATE TABLE public.lodge_email_audit_events (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  event_type text NOT NULL,
  email_account_id uuid REFERENCES public.lodge_email_accounts(id) ON DELETE SET NULL,
  member_id uuid REFERENCES public.lodge_members(id) ON DELETE SET NULL,
  position_id uuid REFERENCES public.lodge_positions(id) ON DELETE SET NULL,
  handover_id uuid REFERENCES public.officer_email_handovers(id) ON DELETE SET NULL,
...
CREATE INDEX lodge_email_audit_events_account_idx
  ON public.lodge_email_audit_events (email_account_id, created_at DESC);
CREATE INDEX lodge_email_audit_events_member_idx
  ON public.lodge_email_audit_events (member_id, created_at DESC);
CREATE INDEX lodge_email_audit_events_handover_idx
  ON public.lodge_email_audit_events (handover_id, created_at DESC)
  WHERE handover_id IS NOT NULL;

CREATE TRIGGER update_lodge_email_accounts_updated_at
  BEFORE UPDATE ON public.lodge_email_accounts
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_officer_email_handovers_updated_at
  BEFORE UPDATE ON public.officer_email_handovers
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_officer_mailbox_assignments_updated_at
  BEFORE UPDATE ON public.officer_mailbox_assignments
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

CREATE OR REPLACE FUNCTION public.enforce_officer_email_handover_transition()
RETURNS trigger
LANGUAGE plpgsql
...
CREATE TRIGGER enforce_officer_email_handover_transition
  BEFORE UPDATE OF state ON public.officer_email_handovers
  FOR EACH ROW EXECUTE FUNCTION public.enforce_officer_email_handover_transition();

CREATE OR REPLACE FUNCTION public.protect_email_policy_history()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = pg_catalog, public
AS $$
...
CREATE TRIGGER protect_email_policy_history
  BEFORE UPDATE OR DELETE ON public.email_policy_versions
  FOR EACH ROW EXECUTE FUNCTION public.protect_email_policy_history();
...
CREATE OR REPLACE FUNCTION public.create_email_policy_version_internal(
...
BEGIN
  ...
  INTO next_version
  FROM public.email_policy_versions
  WHERE policy_type = target_policy_type;

  UPDATE public.email_policy_versions
  SET is_active = false
  WHERE policy_type = target_policy_type
    AND is_active;

  INSERT INTO public.email_policy_versions (
    policy_type,
    title,
    version,
    content,
...
REVOKE ALL ON FUNCTION public.create_email_policy_version_internal(
  text, text, text, text, timestamptz, boolean, uuid
) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_email_policy_version_internal(
  text, text, text, text, timestamptz, boolean, uuid
) TO service_role;
...
INSERT INTO public.officer_mailbox_assignments (
  email_account_id,
  position_id,
  member_id,
  status,
...
CREATE OR REPLACE FUNCTION public.current_lodge_member_id()
RETURNS uuid
LANGUAGE sql
SECURITY DEFINER
STABLE
...
CREATE OR REPLACE FUNCTION public.get_my_lodge_email_accounts()
RETURNS TABLE (
  id uuid,
  address text,
  account_type text,
...
AS $$
  WITH current_member AS (
    SELECT public.current_lodge_member_id() AS id
  )
  SELECT
    account.id,
    account.address,
  ...
  LEFT JOIN LATERAL (
    SELECT version.*
    FROM public.email_policy_versions AS version
  ...
  LEFT JOIN LATERAL (
    ...
    FROM public.email_agreement_acceptances AS receipt
    JOIN public.email_policy_versions AS accepted_policy
...
CREATE OR REPLACE FUNCTION public.get_email_agreement_receipt(target_account_id uuid)
RETURNS TABLE (
  acceptance_id uuid,
  member_name text,
  email_address text,
...
AS $$
DECLARE
  current_member uuid := public.current_lodge_member_id();
BEGIN
  IF current_member IS NULL
    AND NOT public.has_admin_section_permission('members', 'read') THEN
    RAISE EXCEPTION 'Not authorized' USING ERRCODE = '42501';
  END IF;

  IF NOT public.has_admin_section_permission('members', 'read')
    AND NOT EXISTS (
      SELECT 1
      FROM public.email_agreement_acceptances AS own_receipt
  ...
  SELECT
    ...
    policy.acknowledgement,
    policy.content
  FROM public.email_agreement_acceptances AS acceptance
  JOIN public.lodge_members AS member ON member.id = acceptance.member_id
  JOIN public.lodge_email_accounts AS account ON account.id = acceptance.email_account_id
  JOIN public.email_policy_versions AS policy ON policy.id = acceptance.policy_version_id
  LEFT JOIN public.lodge_positions AS position ON position.id = acceptance.position_id
  WHERE acceptance.email_account_id = target_account_id
    AND (
      public.has_admin_section_permission('members', 'read')
...
ALTER TABLE public.lodge_email_accounts ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.email_policy_versions ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.officer_email_handovers ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.officer_mailbox_assignments ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.email_agreement_acceptances ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.email_account_action_tokens ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.lodge_email_audit_events ENABLE ROW LEVEL SECURITY;

GRANT SELECT ON public.lodge_email_accounts TO authenticated;
GRANT SELECT ON public.email_policy_versions TO authenticated;
GRANT SELECT ON public.officer_email_handovers TO authenticated;
GRANT SELECT ON public.officer_mailbox_assignments TO authenticated;
GRANT SELECT ON public.email_agreement_acceptances TO authenticated;
GRANT SELECT ON public.lodge_email_audit_events TO authenticated;

GRANT SELECT, INSERT, UPDATE, DELETE ON public.lodge_email_accounts TO service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.email_policy_versions TO service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.officer_email_handovers TO service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.officer_mailbox_assignments TO service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.email_agreement_acceptances TO service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.email_account_action_tokens TO service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.lodge_email_audit_events TO service_role;

CREATE POLICY "Members can view their Lodge email accounts"
  ON public.lodge_email_accounts FOR SELECT
  TO authenticated
  USING (
    public.has_admin_section_permission('members', 'read')
    OR associated_member_id = public.current_lodge_member_id()
    OR EXISTS (
      SELECT 1
      FROM public.officer_mailbox_assignments AS assignment
      WHERE assignment.email_account_id = lodge_email_accounts.id
        AND assignment.member_id = public.current_lodge_member_id()
        AND assignment.status IN ('PENDING', 'ACTIVE')
    )
  );

CREATE POLICY "Members can view published email policies"
  ON public.email_policy_versions FOR SELECT
  TO authenticated
  USING (is_active OR public.has_admin_section_permission('members', 'read'));

CREATE POLICY "Participants can view mailbox assignments"
  ON public.officer_mailbox_assignments FOR SELECT
  TO authenticated
  USING (
    public.has_admin_section_permission('members', 'read')
    OR member_id = public.current_lodge_member_id()
  );

CREATE POLICY "Participants can view officer email handovers"
  ON public.officer_email_handovers FOR SELECT
  TO authenticated
  USING (
    public.has_admin_section_permission('members', 'read')
    OR outgoing_member_id = public.current_lodge_member_id()
    OR incoming_member_id = public.current_lodge_member_id()
  );

CREATE POLICY "Members can view their agreement receipts"
  ON public.email_agreement_acceptances FOR SELECT
  TO authenticated
  USING (
    public.has_admin_section_permission('members', 'read')
    OR member_id = public.current_lodge_member_id()
...
CREATE POLICY "Members can view their email audit history"
  ON public.lodge_email_audit_events FOR SELECT
  TO authenticated
  USING (
    public.has_admin_section_permission('members', 'read')
    OR member_id = public.current_lodge_member_id()
